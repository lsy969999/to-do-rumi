...
import SQLite
import Tauri
func getAllTodo(db: Connection) throws -> [Todo] {
    let query = tb_todo
        .select(
            tb_todo[*],
            tb_todo_group[*]
...
func getAllTodoCnt(db: Connection) {
    
}

func getTodoPage(db: Connection) {
    
}

func insertTodo(db: Connection, todo: String) throws -> Int64  {
    let rowid = try db.run(tb_todo.insert(todo_todo <- todo))
    return rowid
}

func updateTodo(db: Connection) {
